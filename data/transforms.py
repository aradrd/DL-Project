...
def rfft2(data):
    ...
    data = torch.fft.rfft(data, 2, normalized=False,onesided=False)
...
def fft2(data):
    assert data.size(-1) == 2
    data = ifftshift(data, dim=(-3, -2))
    data = torch.view_as_real(torch.fft.fft2(torch.view_as_complex(data), dim=(-2,-1), norm = "ortho"))
    data = fftshift(data, dim=(-3, -2))
    return data
...
def ifft2_regular(data):
    assert data.size(-1) == 2
    data = ifftshift(data, dim=(-3, -2))
    data = torch.view_as_real(torch.fft.ifft2(torch.view_as_complex(data), dim=(-2,-1), norm='ortho'))
    data = fftshift(data, dim=(-3, -2))
